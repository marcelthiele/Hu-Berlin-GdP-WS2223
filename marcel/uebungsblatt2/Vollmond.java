...
public class Vollmond {
	public static void main(String[] args) {
		...
		String[] foundMoons = findMoons(365, 29, 2, 2022, 1, daysOfMonth, monthNames, n);
		
		//print out foundmoons
//		for(int i = 0; i < foundMoons.length; i++) {
//			System.out.println(foundMoons[i]);
//		}
	...
	/**
	 * @param numOfDaysPerYear Wie viele Tage hat ein Jahr (Ohne, dass es ein Schaltjahr ist)
	 * @param moonPeriod In welcher Periode kommt ein Vollmond auf
	 * @param numOfMoonsPerMonthToFind	wie viele Vollmonde sollen pro Monat aufkommen
	 * @param currentYear Das Aktuelle Jahr
	 * @param startingMoonDayDate	An welchem Tag des Jahres gibt es das erste mal Vollmond
	 * @param daysOfMonth	Wie viele Tage haben die einzelnen Monate
	 * @param numOfMoonsToFind	Wie viele gesuchte Monate sollen gefunden werden
	 */
	private static String[] findMoons(int numOfDaysPerYear, int moonPeriod, int numOfMoonsPerMonthToFind, int currentYear, int startingMoonDayDate, int[] daysOfMonth, String[] monthNames, int numOfMoonsToFind) {
		...
		while(numOfFoundMoons < numOfMoonsToFind) {
			...
			for(; currentDaysCounter <= numOfDaysInCurrentYear && numOfFoundMoons < numOfMoonsToFind; currentDaysCounter += 29) {
				if(isLeapYear && currentDaysCounter >= dayDateOfFirstOfMarch && !corrected) {
					//Wenn nach 29.2. dann müssen wir einen Tag abziehen um den Tag in unseren "interstingDays" zu finden
...
//					retStringArray = addToStringArray(convertDaysToMonth(currentDaysCounter, currentYear, monthNames), retStringArray);
			...
			if(isLeapYear && corrected) currentDaysCounter++;
			
			//Hier werden die "verbleibenden" Tage der Mondperiode in das kommende Jahr übertragen
...
//			retStringArray = addToStringArray("Keine Monate gefunden in denen es " + " Vollmonde mit einer Vollmond-Periode von " + " Tagen gefunden", retStringArray);
	...
	private static int[] getInterestingDays(int[] months, int numOfDaysInYear, int moonPeriod, int numOfMoonsPerMonth) {
		...
		if(checkSum != numOfDaysInYear) return null;
		...
		for(int i = 0; i < months.length; i++) {
			if(months[i] > moonPeriod*(numOfMoonsPerMonth - 1)) {
				for(int j = currentDaysCounter; j < currentDaysCounter + months[i] - (moonPeriod*(numOfMoonsPerMonth - 1)); j++) {					
					interestingDays = addToIntArray(j, interestingDays);
//					System.out.println("currently interesting Days: " + Arrays.toString(interestingDays));
				}
			}
	 ...
	 * @param monthNames	Array of the Names of months (eg Januar, Februar, ..., Dezember)
...
//		System.out.println("arrayIndex: "+arrayIndex + ", i: "+i);
	 ...
	 * @return	einen neuen Array, der das neue Element am Ende enthält. Hat eine Länge von prevArray.length+1
	 */
